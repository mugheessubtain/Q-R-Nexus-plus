...
void DataService::init() {
    ...
    alpacaApiKey = config.value("alpaca_api_key", "");
    alpacaApiSecret = config.value("alpaca_api_secret", "");

    // Example map initialization for DSA highlight
    symbolMap["BTCUSDT"] = "Binance";
    symbolMap["ETHUSDT"] = "Binance";
    symbolMap["AAPL"] = "Alpaca";
}

// Binary search example: find closest candle by timestamp
models::Candle DataService::binarySearchCandle(const std::vector<models::Candle>& candles, long long timestamp) {
    int left = 0, right = candles.size() - 1;
    while (left <= right) {
        int mid = left + (right - left) / 2;
        if (candles[mid].time == timestamp)
            return candles[mid];
        else if (candles[mid].time < timestamp)
            left = mid + 1;
        else
            right = mid - 1;
    }
    // If exact timestamp not found, return closest left
    return candles[std::max(0, right)];
}

// Example of sliding window using deque (rolling moving average)
double calculateRollingMA(const std::deque<double>& window) {
    double sum = 0.0;
